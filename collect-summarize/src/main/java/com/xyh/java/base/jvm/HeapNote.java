...
public class HeapNote {
	...
	3.永久代
	永久代是Hotspot虚拟机特有的概念，是方法区的一种实现，别的JVM都没有这个东西。在Java 8中，永久代被彻底移除，取而代之的是另一块与堆不相连的本地内存——元空间（meta-space）。
	永久代或者“Perm Gen”包含了JVM需要的应用元数据，这些元数据描述了在应用里使用的类和方法。注意，永久代不是Java堆内存的一部分。永久代存放JVM运行时使用的类。
	...
	{
		方法区:出自java虚拟机规范, 可供各条线程共享的运行时内存区域。它存储了每一个类的结构信息，
		例如运行时常量池（Runtime Constant Pool）、字段和方法数据、构造函数和普通方法的字节码内容。
		上面讲的是规范，在不同虚拟机里头实现是不一样的，最典型的就是永久代(PermGen space)和元空间(Metaspace)。
		jdk1.8以前:实现方法区的叫永久代。因为在很久远以前，java觉得类几乎是静态的，并且很少被卸载和回收，所以给了一个永久代的雅称。
		因此，如果你在项目中，发现堆和永久代一直在不断增长，没有下降趋势,回收的速度根本赶不上增长的速度，不用说了，这种情况基本可以确定是内存泄露。
		jdk1.8以后:实现方法区的叫元空间。Java觉得对永久代进行调优是很困难的。永久代中的元数据可能会随着每一次Full GC发生而进行移动。
		并且为永久代设置空间大小也是很难确定的。因此，java决定将类的元数据分配在本地内存中，元空间的最大可分配空间就是系统可用内存空间。
		这样，我们就避开了设置永久代大小的问题。但是，这种情况下，一旦发生内存泄露，会占用你的大量本地内存。
		如果你发现，你的项目中本地内存占用率异常高。嗯，这就是内存泄露了。
	}
 */
